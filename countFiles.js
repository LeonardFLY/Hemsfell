...
const fileCounts = {};
const filePath = 'fileCounts.json';
    ...
    return new Promise((resolve, reject) => {
        fs.access(filePath, fs.constants.F_OK, (err) => {
            if (!err) {
                ...
                // Arquivo não existe, então resolve diretamente
                resolve();
            }
        });
    });
...
const processFolders = async () => {
    try {
        ...
        fs.writeFileSync(filePath, JSON.stringify(fileCounts, null, 2));
        console.log('fileCounts.json gerado com sucesso!');
    } catch (err) {
